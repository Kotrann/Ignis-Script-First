...
local showHealthBar = false -- Показывать HP бар
local showNames = false -- Показывать имена
...
local function createMenu()
    ...
    mainFrame.Visible = true  -- ← ИЗМЕНЕНО: сразу показываем меню
    ...
    local function createCheckbox(parent, text, defaultValue, callback)
        ...
        checkbox.BackgroundColor3 = defaultValue and Color3.fromRGB(255, 140, 0) or Color3.fromRGB(60, 60, 60) -- Orange вкл/тёмно-серый выкл
...
local function createESP(character)
    ...
    if showHealthBar then
        ...
        healthBarBG.Size = UDim2.new(1, 0, 0, healthBarHeight)
        healthBarBG.Position = UDim2.new(0, 0, 0, nameSize + 2)
...
local function addESP(character)
    ...
    -- Create ESP элементы
    local espFolder, billboard = createESP(character)
        ...
        if fovCircle and fovCircle.Parent then
            if not aimEnabled or not showFOV then 
                ...
                if aiming and lockedTarget then
                    fovCircle:FindFirstChildOfClass("UIStroke").Color = Color3.fromRGB(0, 255, 0)
                else
                    fovCircle:FindFirstChildOfClass("UIStroke").Color = Color3.fromRGB(255, 255, 255)
                end
            end
        end
    end
